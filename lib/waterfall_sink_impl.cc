  ...
  namespace strf
  {
            ...
            d_fft (fft_size)
    {
      ...
      for(size_t i = 0; i < fft_size; i++) {
        r = (samp_rate/fft_size * i ) - samp_rate/2.0;
        d_fos.write((char *)&r, sizeof(float));
      }

      /* Get start time */
      gettimeofday(&d_tstart, 0);
      tstart = d_tstart;
      tend = d_tstart;
      ...
      uint32_t rint = 0;
      rint = d_tstart.tv_sec;
      d_fos.write((char *)&rint, sizeof(uint32_t));
      rint = d_tstart.tv_usec;
      d_fos.write((char *)&rint, sizeof(uint32_t));
    }
    ...
    waterfall_sink_impl::compute_decimation (const gr_complex* in, size_t n_fft)
    {
	  ...
	  tlen = (tend.tv_sec-tstart.tv_sec)+(float) (tend.tv_usec-tstart.tv_usec)*1e-6;
	  t = 0.5*(tend.tv_sec+tstart.tv_sec)-d_tstart.tv_sec;
	  t += (0.5*(float) (tend.tv_usec+tstart.tv_usec)-(float) d_tstart.tv_usec)*1e-6;	  
